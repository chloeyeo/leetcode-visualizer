...
async function callClaude(key, model, system, user, maxTokens) {
  ...
  if (!res.ok) throw new Error(`Claude ${res.status}: ${(await res.text().catch(() => '')).slice(0, 180)}`);
...
async function callGemini(key, model, system, user, maxTokens) {
  ...
  const res = await fetch(url, {
    ...
    body: JSON.stringify({
      ...
      generationConfig: { maxOutputTokens: maxTokens, temperature: 0.6, responseMimeType: 'application/json' },
    }),
  });
  if (!res.ok) throw new Error(`Gemini ${res.status}: ${(await res.text().catch(() => '')).slice(0, 180)}`);
  const data = await res.json();
