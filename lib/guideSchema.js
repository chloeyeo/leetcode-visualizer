...
function validateRule(r, i, errs) {
  ...
  // Optional scope: evaluate the rule inside one function's subtree only —
  // needed when the starter ships helpers that would pre-satisfy the rule.
  if (r.within !== undefined && (r.kind === 'defines' || !isStr(r.within, 60))) {
    errs.push(`${at}: within must be a function-name string (not valid on "defines")`);
  }
